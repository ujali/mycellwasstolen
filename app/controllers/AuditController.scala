...
class AuditController(auditRepo: AuditRepository) extends Controller with Secured {
  ...
  def timestampForm: Action[AnyContent] = withAuth { username =>
    implicit request =>
      ...
      val list = List()
      Ok(views.html.admin.contents.mobileCheckStatusTimestamp("imeid", list, user))
  }
  ...
  def timestampsByIMEI: Action[AnyContent] = withAuth { username =>
    implicit request =>
      ...
      timestampform.bindFromRequest().fold(
        hasErrors = { form =>
          val list = List()
          Ok(views.html.admin.contents.mobileCheckStatusTimestamp("imeid", list, user)).flashing("error" -> "Please correct the errors in the form")
        },
        success = { timestamp =>
  ...
  def allTimestamps: Action[AnyContent] = withAuth { username =>
    implicit request =>
      ...
      Ok(views.html.admin.contents.mobileCheckStatusTimestamp("all", list, user))
  }
  ...
  def registrationRecordsByYear(year: String): Action[AnyContent] = withAuth { username =>
    implicit request =>
      ...
      Ok(views.html.admin.contents.analytics(user, recordsList, years))
  }
}
