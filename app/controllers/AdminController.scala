 ...
 */
class AdminController(mobileRepo: MobileRepository, brandRepo: BrandRepository, modelRepo: ModelRepository, auditRepo: AuditRepository, mail: MailUtil, s3Util: S3UtilComponent) extends Controller with Secured {
  ...
  def index: Action[AnyContent] = withAuth { username =>
    implicit request =>
      ...
      Ok(views.html.admin.index("Welcome", user))
  }
  ...
  def mobiles(status: String): Action[AnyContent] = withAuth { username =>
    implicit request =>
      ...
      Ok(views.html.admin.contents.mobiles(status, mobiles, user))
  }
